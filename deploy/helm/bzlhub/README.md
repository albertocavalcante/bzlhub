...
A Helm chart for deploying [bzlhub](https://github.com/albertocavalcante/bzlhub)
— the Bazel-first self-hosted module registry — onto any Kubernetes
cluster.

## Status & scope

- **Single-replica**, single-writer. bzlhub uses SQLite + a local
...
```sh
git clone https://github.com/albertocavalcante/bzlhub
helm install bzlhub ./bzlhub/deploy/helm/bzlhub
```

### 2) Public registry (after publish)

```sh
helm install bzlhub oci://ghcr.io/albertocavalcante/charts/bzlhub --version 0.1.0
```
...
```sh
helm install bzlhub ./bzlhub/deploy/helm/bzlhub -f values-prod.yaml
```
...
${CONTAINER:-docker} build -t bzlhub:dev /path/to/bzlhub

# Make it visible to the cluster.
kind load docker-image bzlhub:dev
# or: minikube image load bzlhub:dev

helm install bzlhub /path/to/bzlhub/deploy/helm/bzlhub \
  --set image.registry="" \
...
`image.registry=""` produces a bare `bzlhub:dev` reference with no
registry prefix, which is what `kind`/`minikube` need for sideloaded
images.

## Building the image

The chart consumes any OCI-compliant bzlhub image. Build it with either
Docker or Podman — the `Dockerfile` deliberately avoids BuildKit-only
features:

```sh
docker build -t bzlhub:dev .
podman build -t bzlhub:dev .
```
...
```sh
# Docker buildx
docker buildx build --platform linux/arm64,linux/amd64 -t ghcr.io/you/bzlhub:0.1.0 --push .

# Podman + manifest
podman build --platform linux/arm64,linux/amd64 --manifest ghcr.io/you/bzlhub:0.1.0 .
podman manifest push ghcr.io/you/bzlhub:0.1.0
```
...
```sh
helm template bzlhub ./deploy/helm/bzlhub --set persistence.mirror.enabled=false --set persistence.index.enabled=false > /tmp/bzlhub.yaml
podman kube play /tmp/bzlhub.yaml
```
...
bzlhub serves Server-Sent Events on `/api/events`. nginx-ingress
...
Optional CronJobs hit bzlhub's HTTP API from inside the cluster
network — `POST /api/ingest-recursive` for periodic mirror refresh,
`GET /api/drift` for daily drift reports. Both reuse the bzlhub image
...
MIT. See the bzlhub repo `LICENSE`.
