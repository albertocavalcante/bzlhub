// Package diff implements the `canopy diff` + `canopy diff-closure`
...
// exitCodeBreaking is what `canopy diff --fail-if-breaking` returns when
...
// when --db is unset. Match cmd/canopy/main.go's defaultDBPath.
const DefaultDBPath = "canopy.db"

// NewCmd builds the `canopy diff` subcommand.
...
// NewClosureCmd builds the `canopy diff-closure` subcommand.
...
pair is analyzed via the same local-or-upstream fallback as 'canopy diff';
