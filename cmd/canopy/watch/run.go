...
import (
	...
	"github.com/albertocavalcante/canopy/internal/forgewatch"
...
func runWatch(ctx context.Context, f watchFlags) error {
	...
	logger.Info("canopy watch starting",
	...
	if err := w.Run(ctx); err != nil && !errors.Is(err, context.Canceled) {
		return fmt.Errorf("canopy watch: %w", err)
	}
	logger.Info("canopy watch stopped")
	return nil
}
...
func preflightWatchForge(ctx context.Context, cfg watchConfig) (bigorna.Forge, error) {
	...
	if err := forgeClient.Health(ctx); err != nil {
		return nil, fmt.Errorf("canopy watch: forge health check failed: %w", err)
	}
