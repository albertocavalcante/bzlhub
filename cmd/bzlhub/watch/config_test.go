...
func clearWatchEnv(t *testing.T) {
	...
	for _, k := range []string{
		...
		"BZLHUB_FORGEJO_TOKEN", "BZLHUB_DB_PATH", "BZLHUB_WATCH_STATE_FILE",
		"BZLHUB_WATCH_INTERVAL",
	} {
		t.Setenv(k, "")
	}
}
...
func TestResolveWatchConfig_IntervalFromEnv(t *testing.T) {
	clearWatchEnv(t)
	t.Setenv("FAKE_TOKEN", "ghp_abc")
	t.Setenv("BZLHUB_WATCH_INTERVAL", "45s")
	wt := gitWorktree(t)
	sf := filepath.Join(t.TempDir(), "state.json")
	f := baseWatchFlags(wt, sf)
	f.interval = 0
...
func TestResolveWatchConfig_BadIntervalEnv(t *testing.T) {
	clearWatchEnv(t)
	t.Setenv("FAKE_TOKEN", "ghp_abc")
	t.Setenv("BZLHUB_WATCH_INTERVAL", "not-a-duration")
	wt := gitWorktree(t)
	sf := filepath.Join(t.TempDir(), "state.json")
	f := baseWatchFlags(wt, sf)
	f.interval = 0
	...
	_, err := resolveWatchConfig(f)
	if err == nil || !strings.Contains(err.Error(), "BZLHUB_WATCH_INTERVAL") {
		t.Fatalf("want BZLHUB_WATCH_INTERVAL parse error, got %v", err)
	}
}
