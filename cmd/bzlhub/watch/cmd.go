// Package watch implements the `bzlhub watch` subcommand: a foreground
...
// NewCmd builds the `bzlhub watch` subcommand.
...
same env + flag layering as 'bzlhub publish'.
...
Stop the daemon with SIGINT / SIGTERM.`,
	...
	cmd.Flags().StringVar(&f.worktree, "worktree", "", "git working clone of the registry repo (env BZLHUB_REGISTRY_WORKTREE)")
	cmd.Flags().StringVar(&f.forge, "forge", "", "forge kind: github (default) | gitlab | bitbucketdc | forgejo (env BZLHUB_FORGE_KIND)")
	cmd.Flags().StringVar(&f.repo, "repo", "", "forge repo identifier (env BZLHUB_FORGE_REPO)")
	cmd.Flags().StringVar(&f.baseURL, "base-url", "", "forge API base URL (env BZLHUB_FORGE_BASE_URL; required for bitbucketdc and forgejo)")
	...
	cmd.Flags().StringVar(&f.baseBranch, "base-branch", "", "branch to watch (env BZLHUB_REGISTRY_BASE_BRANCH; default main)")
