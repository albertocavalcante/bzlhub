...
func buildPublisher(cfg publishConfig) (publish.Publisher, func(), error) {
	...
	case cfg.dryRun:
		...
		if err != nil {
			cleanup()
			return nil, noop, fmt.Errorf("bzlhub publish: init scratch filesystem: %w", err)
		}
	...
	case cfg.commitMode:
		...
		if err != nil {
			return nil, noop, fmt.Errorf("bzlhub publish: init git-direct publisher: %w", err)
		}
		return gd, noop, nil
	default:
		...
		if err != nil {
			return nil, noop, fmt.Errorf("bzlhub publish: init git-pr publisher: %w", err)
		}
		return gp, noop, nil
	}
}
