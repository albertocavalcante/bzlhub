// publish.go — bzlhub publish command. See docs/plans/11-forge-library.md
...
// NewCmd builds the `bzlhub publish` subcommand.
...
For pre-made source.json from disk:
  ...
  bzlhub publish mylib@2.0.0 --source-json ./source.json
...
env vars by default. Use --dry-run to validate config without pushing.`,
	...
	cmd.Flags().StringVar(&f.repo, "repo", "", "forge repo identifier; <owner>/<name> for github (env BZLHUB_FORGE_REPO)")
