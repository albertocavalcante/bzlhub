...
func clearPublishEnv(t *testing.T) {
	...
	for _, k := range []string{
		"BZLHUB_REGISTRY_WORKTREE", "BZLHUB_FORGE_KIND", "BZLHUB_FORGE_BASE_URL",
		"BZLHUB_REGISTRY_BASE_BRANCH", "BZLHUB_FORGE_REPO", "BZLHUB_FORGE_TOKEN_ENV",
		"BZLHUB_GITHUB_TOKEN", "BZLHUB_GITLAB_TOKEN", "BZLHUB_BITBUCKET_TOKEN",
		"BZLHUB_FORGEJO_TOKEN", "BZLHUB_BOT_NAME", "BZLHUB_BOT_EMAIL",
	} {
		t.Setenv(k, "")
	}
}
...
func TestResolvePublishConfig_FlagBeatsEnv(t *testing.T) {
	clearPublishEnv(t)
	t.Setenv("BZLHUB_REGISTRY_WORKTREE", "/from/env")
	t.Setenv("BZLHUB_FORGE_REPO", "envo/envr")
	t.Setenv("FAKE_TOKEN", "ghp_abc")
...
func TestResolvePublishConfig_BitbucketDCTokenDefault(t *testing.T) {
	clearPublishEnv(t)
	t.Setenv("BZLHUB_BITBUCKET_TOKEN", "bbdc-secret")
	f := basePublishFlags("/tmp/w")
	f.forge = "bitbucketdc"
	...
	if cfg.tokenEnv != "BZLHUB_BITBUCKET_TOKEN" || cfg.token != "bbdc-secret" {
...
func TestResolvePublishConfig_GitLabTokenDefault(t *testing.T) {
	clearPublishEnv(t)
	t.Setenv("BZLHUB_GITLAB_TOKEN", "glpat-secret")
	f := basePublishFlags("/tmp/w")
	f.forge = "gitlab"
	f.tokenEnv = "" // exercise the per-forge default selection
	cfg, err := resolvePublishConfig(f)
	...
	if cfg.tokenEnv != "BZLHUB_GITLAB_TOKEN" || cfg.token != "glpat-secret" {
...
func TestResolvePublishConfig_ForgejoTokenDefaultAndBaseURLRequired(t *testing.T) {
	clearPublishEnv(t)
	t.Setenv("BZLHUB_FORGEJO_TOKEN", "fjo-secret")
	f := basePublishFlags("/tmp/w")
	f.forge = "forgejo"
	f.tokenEnv = "" // exercise the per-forge default selection
	...
	// With base-url set, the default token env is BZLHUB_FORGEJO_TOKEN.
	f.baseURL = "https://codeberg.org"
	cfg, err := resolvePublishConfig(f)
	if err != nil {
		t.Fatalf("resolve: %v", err)
	}
	if cfg.tokenEnv != "BZLHUB_FORGEJO_TOKEN" || cfg.token != "fjo-secret" {
