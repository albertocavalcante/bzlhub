...
// `canopy drift refresh` precisely when they want fresh data over
// any prior verdict — typically right after `canopy sync bootstrap`,
// before `canopy serve` would naturally re-warm at next boot.
...
// a silent no-op — `canopy drift refresh` against a File-backed
