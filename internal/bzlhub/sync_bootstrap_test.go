...
// TestSyncBootstrap_AlreadyBootstrappedReceiptPopulatesDuration
// asserts the consistency contract: when ErrAlreadyBootstrapped
// fires, the receipt still carries Duration (time spent on
// detection). Otherwise the operator inspecting "how long did
// this take" sees zero on the no-op path but real values on the
// success path — confusing asymmetry.
func TestSyncBootstrap_AlreadyBootstrappedReceiptPopulatesDuration(t *testing.T) {
	ctx := t.Context()
	svc := newTestService(t)
	...
	if rec.SHA == "" {
		t.Errorf("receipt.SHA is empty on ErrAlreadyBootstrapped; expected the existing HEAD")
	}
	if rec.Duration == 0 {
		t.Errorf("receipt.Duration is zero on ErrAlreadyBootstrapped; expected detection time")
	}
}
...
// remote URL) still produces an audit row, with OK=false, the
// error message captured, and the payload carrying remote +
// mirror_path so a post-mortem reviewer can reconstruct what was
// attempted.
func TestSyncBootstrap_AuditOnFailure(t *testing.T) {
	...
	target := filepath.Join(t.TempDir(), "mirror")
	const badRemote = "file:///definitely/not/a/remote"
	_, err := svc.SyncBootstrap(ctx, SyncBootstrapOptions{
		Remote:     badRemote,
		MirrorPath: target,
		Branch:     "master",
	...
	if payload.Remote != badRemote {
		t.Errorf("payload remote = %q; want %q", payload.Remote, badRemote)
	}
	if payload.MirrorPath != target {
		t.Errorf("payload mirror_path = %q; want %q", payload.MirrorPath, target)
	}
	if payload.Branch != "master" {
		t.Errorf("payload branch = %q; want master", payload.Branch)
	}
}
