...
func registerCodenavTools(srv *server.MCPServer, c api.Canopy) {
	srv.AddTool(
		mcp.NewTool("canopy_lookup_symbol",
			mcp.WithDescription("Resolve a SCIP symbol string to its definition site (file + range + documentation) for a specific (module, version). Backed by understory v0.1.0, the OSS code-navigation library canopy embeds.\n\nThe SCIP symbol shape canopy emits is `bzlmod <module>@<version> <relpath>#<name>` — e.g. `bzlmod rules_python@0.40.0 python/defs.bzl#py_library`. You construct the symbol by combining the (module, version) coordinate with the rule/provider/macro's provenance.file (returned by canopy_get_module) and its short name.\n\nReturns Found=false (NOT an error) when the symbol isn't defined in this index — typical when the symbol is present only as an external `load(...)` reference (which means it's defined in a DIFFERENT module's SCIP; call canopy_lookup_symbol again with that module's coordinates).\n\nUse this when you need the AUTHORITATIVE location for a symbol with cross-module precision. For broad lookups (rules/providers in a module), prefer canopy_get_module — its ModuleReport already carries provenance and is cheaper."),
	...
	srv.AddTool(
		mcp.NewTool("canopy_lookup_references",
			...
			mcp.WithString("symbol", mcp.Required(), mcp.Description("Full SCIP symbol string (same shape as canopy_lookup_symbol).")),
