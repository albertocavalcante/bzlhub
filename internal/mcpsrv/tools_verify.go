...
// registerVerifyTools registers the canopy_verify tool. Called only
// when Serve was given a non-nil Verifier (the in-process verify
// binding only exists when canopy serve/mcp was wired with a mirror
// root + db path).
func registerVerifyTools(srv *server.MCPServer, v Verifier) {
	srv.AddTool(
		mcp.NewTool("canopy_verify",
